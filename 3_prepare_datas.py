...
"""#### Split data into trainA, trainB & trainC for hold out stacking """
trainA, trainBC = train_test_split(train, train_size=0.4, stratify = train[target],random_state=SEED)
trainB, trainC = train_test_split(trainBC, train_size=0.5, stratify = trainBC[target],random_state=SEED)
trainA.to_csv('data/trainA.csv', index = False)
trainB.to_csv('data/trainB.csv', index = False)
trainC.to_csv('data/trainC.csv', index = False)

print('TrainA Shape: ' + str(trainA.shape))
trainA_country_distribution = get_percentage(trainA, target)
trainA_country_distribution.to_csv('data/country_distribution_trainA.csv', index = False)
print('trainB Shape: ' + str(trainB.shape))
trainB_country_distribution = get_percentage(trainB, target)
trainB_country_distribution.to_csv('data/country_distribution_trainB.csv', index = False)
print('trainB Shape: ' + str(trainB.shape))
trainC_country_distribution = get_percentage(trainC, target)
trainC_country_distribution.to_csv('data/country_distribution_trainC.csv', index = False)

# TrainA Shape: (23620, 85)
#     country_destination  Count         %
# 0                     7  14413  0.610203
# 1                    10   6430  0.272227
# 2                    11   1170  0.049534
