...
def get_matrix(y_test, y_test_pred, y_train, y_train_pred, estimator_name, label_encoder):
    check_dir(SAMPLED_TRAIN_RESULT_PATH)
    ...
    df_confusion.to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH, convert_title_to_filename(title) + '.csv'))
...
def train_clf(clf, x_train, x_test):
    ...
    pd.DataFrame(clf.predict_proba(x_train_transformed)).to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,"Prob_Train_{}.csv").format(clf_name), index = False)
    pd.DataFrame(clf.predict_proba(x_test_transformed)).to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,"Prob_Test_{}.csv").format(clf_name), index = False)

    score_df = pd.DataFrame(data = {'Prediction': ['Train Prediction', 'Test Prediction'], 'Accuracy Score': accuracy_scores})
    score_df.to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,'Scores_for_{}.csv'.format(clf_name)), index = False)
...
def get_feature_importance_by_tree(clf):
    ...
    feature_importance.to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,'Feature_Importance_{}.csv').format(clf.__class__.__name__), index = False)

    return feature_importance

import pickle
def save_obj(obj, name):
    with open(os.path.join(SAMPLED_TRAIN_RESULT_PATH, name + '.pkl'), 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)

def load_obj(name):
    with open(os.path.join(SAMPLED_TRAIN_RESULT_PATH, name + '.pkl'), 'rb') as f:
...
print("Before OverSampling, Distribution of Target: \n{}".format(get_percentage(pd.DataFrame(data = {target: y_train}), target)))
...
print("After OverSampling, Distribution of Target: \n{}".format(get_percentage(pd.DataFrame(data = {target: y_train_smt}), target)))

xgb = XGBClassifier(max_depth= 6, learning_rate=0.3, n_estimators=25,
                    objective='multi:softprob', subsample=0.5, colsample_bytree=0.5)

xgb.fit(x_train_smt, y_train_smt)
accuracy_score(xgb.predict(x_train_smt), y_train_smt)
accuracy_score(xgb.predict(x_train), y_train)
accuracy_score(xgb.predict(x_test.values), y_test)

rfc = RandomForestClassifier(min_samples_split = 400, n_estimators=100)
rfc.fit(x_train_smt, y_train_smt)
accuracy_score(rfc.predict(x_train_smt), y_train_smt)
accuracy_score(rfc.predict(x_test.values), y_test)
...
# start_total = time.perf_counter()
#
# checkpoint = ModelCheckpoint(os.path.join(SAMPLED_MODEL_PATH, 'NeuralNetwork.hdf5'),
#                             monitor='val_acc', verbose=1, save_best_only=True, mode='max')
# model = get_model()
# history = model.fit(x_train_values, y_train_cat, epochs=100, batch_size=32,
#                     callbacks=[EarlyStopping(monitor='val_loss', patience=5, verbose=0),
#                     ModelCheckpoint(os.path.join(SAMPLED_MODEL_PATH, 'NeuralNetwork.hdf5'),
...
#
# model.save(os.path.join(SAMPLED_MODEL_PATH, 'nn_1.h5')) # save model
#
...
# check_dir(SAMPLED_TRAIN_RESULT_PATH)
# (pd.DataFrame(y_train_prob)).to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,"Prob_Train_{}.csv").format(clf_name), index = False)
# (pd.DataFrame(y_test_prob)).to_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH,"Prob_Test_{}.csv").format(clf_name), index = False)
#
...
# """#### Model Comparison"""
# # Runtime
# run_times_df.transpose().rename(columns = {0: 'Run Time (sec)'}).plot(kind='line', figsize=(12, 8))
# plt.title('Runtime Across Models', loc = 'center', y=1.1, fontsize = 25)
#
# # Accuracy
# score_filenames = [file for file in os.listdir(SAMPLED_TRAIN_RESULT_PATH) if file.startswith('Scores_for_')]
# score_dict = {}
# for filename in score_filenames:
#     estimatorName = filename.replace('Scores_for_', '').replace('.csv', '')
#     score_df = pd.read_csv(os.path.join(SAMPLED_TRAIN_RESULT_PATH, filename))
...
# plt.ylim(0.6, 0.7)
# plt.legend()
# plt.title('Accuracy Scores Across Models', loc = 'center', y=1.1, fontsize = 25)
